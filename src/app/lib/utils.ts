...
export const generateChartData = () => {
    ...
    return weeks.map(week => ({
        weekOfYear: week, // Keeping 'day' key for compatibility with existing chart components
        count: Math.floor(Math.random() * 51) + 10 // Random applications between 10 and 60 for weekly data
    }));
};
